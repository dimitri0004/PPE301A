...
class DirecteurController extends AbstractController
{
    ...
    public function update($id,Directeur $directeur,DirecteurRepository $directeurRepository, Request $request, EntityManagerInterface $entityManagerInterface): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()){
            ...
            $directeur->setDateMiseaJour($updateDate);
            $directeurRepository->save($directeur,flush:true);
            $entityManagerInterface->persist($directeur);
            $entityManagerInterface->flush();
